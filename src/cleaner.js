...
$( document ).ready(function() {
  ...
  $("#format-para").click(function() {
    ...
    $start.nextUntil($end.next()).andSelf().wrapAll("<p/>");
    ...
    $.ajax(API+'/auth/whoami/' + window.localStorage.getItem('token'), {
      ...
      success: function(whoami) {
        if (whoami.user) {
            ...
            var event = new CustomEvent("ga", {"detail":{"origin":"HA-Cleaner","type":"XHR","action":"User logged in (whoami)"}});
            ...
            var event = new CustomEvent("ga", {"detail":{"origin":"HA-Cleaner","type":"XHR","action":"User NOT logged in (whoami)"}});
            document.dispatchEvent(event);
        }
